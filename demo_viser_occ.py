...
def load_model(checkpoints_folder, checkpoint_file):
...
def main():
    checkpoints_folder_sd = "checkpoints/normalized"  # specify your checkpoints folder
    checkpoint_file_sd = "ckpt.pt"  # specify your checkpoint file 
    checkpoints_folder = "checkpoints/shapenet_geom_012"  # specify your checkpoints folder
    checkpoint_file = "epoch_1000.pt"  # specify your checkpoint file 
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    path_to_point_cloud = "data/ase/chair4.ply"  # specify your input point cloud path
    z_up = True  # specify if your input point cloud is in z-up orientation
    ...
    model = load_model(checkpoints_folder, checkpoint_file)
    model_sd = load_model(checkpoints_folder_sd, checkpoint_file_sd)
    ...
    with torch.no_grad():
        ...
        outdict_sd = model_sd(points)
        for key in outdict:
            if isinstance(outdict[key], torch.Tensor):
                outdict[key] = outdict[key].cpu()
        for key in outdict_sd:
            if isinstance(outdict_sd[key], torch.Tensor):
                outdict_sd[key] = outdict_sd[key].cpu()
        translation = np.zeros(3)
        scale = 1.0
        translation = np.array([translation])
        scale = np.array([scale])
        outdict = denormalize_outdict(outdict, translation, scale, z_up)
        outdict_sd = denormalize_outdict(outdict_sd, translation, scale, z_up)
    ...
    pred_handler_sd = PredictionHandler.from_outdict(outdict_sd, points, ['chair'])
    mesh_sd = pred_handler_sd.get_meshes(resolution=resolution)[0]

    server = viser.ViserServer()
    server.scene.add_mesh_trimesh("superquadrics", mesh=mesh, visible=True)
    server.scene.add_mesh_trimesh("superquadrics_sd", mesh=mesh_sd, visible=True)
    
    points = np.array(pcs.points)
    dark_gray = np.array([0.5, 0.5, 0.5])
    colors = np.tile(dark_gray, (points.shape[0], 1))
