...
class Phase2Evaluator:
    def __init__(self, cfg, model, loss_fn, dataset, device=None):
        ...
        self.split = cfg_get(cfg_get(cfg, "dataset"), "split", "test")
        self.output_dir = Path(cfg_get(eval_cfg, "output_dir", "gendec/data/eval")) / cfg_get(
            cfg, "run_name", "gendec_phase2_eval"
        )
        self.autodec_decode_cfg = cfg_get(cfg, "autodec_decode")
        self._frozen_autodec_decoder = None
    ...
    def _write_generated_visualizations(self, generated, generated_autodec=None):
        ...
        visualizer = GeneratedSQVisualizer(
            root_dir=cfg_get(vis_cfg, "root_dir", "data/viz"),
            run_name=cfg_get(self.cfg, "run_name", "gendec_phase2_eval"),
            mesh_resolution=cfg_get(vis_cfg, "mesh_resolution", 24),
            exist_threshold=cfg_get(vis_cfg, "exist_threshold", 0.5),
            max_preview_points=cfg_get(vis_cfg, "max_preview_points", 4096),
        )
    ...
    def _sample_generated(self, num_samples):
        ...
        residual_dim = int(cfg_get(model_cfg, "residual_dim", 64))
        return sample_joint_scaffolds(
            model=self.model,
            stats=self.dataset.stats,
            num_samples=int(num_samples),
    ...
    def evaluate(self):
        ...
        if "tokens_z" in generated:
            residual_norm = generated["tokens_z"].pow(2).mean(dim=-1).sqrt()
            metrics["generated_residual_norm_mean"] = float(residual_norm.mean().detach().cpu().item())
        metrics["num_rows"] = len(per_sample_rows)

        generated_autodec = None
        if self._autodec_decode_enabled():
            bridge = self._get_frozen_autodec_decoder()
            generated_autodec = decode_joint_scaffolds(
                generated,
                decoder=bridge["decoder"],
            )
        ...
        torch.save(
            {
                "tokens": generated["tokens"].cpu(),
                "tokens_z": generated["tokens_z"].cpu(),
                "tokens_ez": generated["tokens_ez"].cpu(),
                "exist": generated["exist"].cpu(),
                "active_mask": generated["active_mask"].cpu(),
                "preview_points": generated["preview_points"].cpu(),
            },
