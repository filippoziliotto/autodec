"""
For each existing primitive, keep the two bending axes with the largest |k|
and zero out the weakest axis. Tapering is still thresholded as in threshold_npz.py.
...
def apply_double_axis_bending(handler: PredictionHandler, thresh_tapering: float) -> None:
    """Modify handler in-place: keep the two strongest bending axes per primitive.
    Tapering is zeroed when max |tapering| < thresh_tapering."""
    ...
    # For each primitive, find the axis with the smallest |k| (the one to drop)
    worst_axis = np.argmin(np.abs(bending_k), axis=-1)  # [B, P]

    for i in range(3):
        # Zero only the weakest axis, for existing primitives
        mask = existing & (worst_axis == i)
        handler.bending[..., 2 * i][mask] = 0.0      # zero k_i
        handler.bending[..., 2 * i + 1][mask] = 0.0  # zero a_i

    # Stats: how many primitives had each axis dropped
    axis_names = ['z', 'x', 'y']
    for i, name in enumerate(axis_names):
        count = int((existing & (worst_axis == i)).sum())
        print(f"  {count}/{total_existing} existing primitives dropped bending on {name}-axis")
...
def main():
    parser = argparse.ArgumentParser(
        description="Keep the two dominant bending axes per primitive in a superdec NPZ file"
    )
    ...
    parser.add_argument("--suffix", type=str, default="_double_axis",
                        help="Suffix appended to the output filename (default: _double_axis)")
    ...
    apply_double_axis_bending(handler, args.thresh_tapering)
